...
import { ArrowLeft, ChevronRight, CheckCircle2, Clock, FileText, Star, Download, Shield, ShieldAlert, type LucideIcon } from "lucide-react";
import GradeEvaluationModal from "@/components/courses/GradeEvaluationModal";
import IntegrityReviewPanel from "@/components/courses/IntegrityReviewPanel";
import type { IntegrityEventItem } from "@/lib/integrity-scoring";
...
export default function SubmissionsClient({
  ...
  const [activeTab, setActiveTab] = useState<"SUBMISSIONS" | "INTEGRITY">("SUBMISSIONS");
  ...
  const studentFlagsMap = useMemo(() => {
    const counts = new Map<string, number>();
    for (const ev of integrityEvents) {
      counts.set(ev.studentId, (counts.get(ev.studentId) || 0) + 1);
    }
    return counts;
  }, [integrityEvents]);
        ...
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-semibold text-slate-900 dark:text-[#F0F2F8]">
              {activeTab === "INTEGRITY" ? "Integrity & Proctoring" : "Submissions"}
            </span>
          </div>

          {enableIntegrityMonitoring && (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-orange-500/10 text-orange-700 dark:text-orange-400 border border-orange-500/20">
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-2 border-b border-slate-200/80 dark:border-white/10 pb-3">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold transition-all ${
              activeTab === "SUBMISSIONS"
                ...
                : "text-slate-600 dark:text-[#8B92A5] hover:bg-slate-100 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-white"
            }`}
          >
          ...
          >
            ...
            <span>Integrity & Proctoring</span>
                    ...
                    <td className="px-5 py-4">
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {(studentFlagsMap.get(row.student.id) || 0) > 0 && (
                            <button
                              type="button"
                              onClick={(e) => {
                                e.stopPropagation();
                              ...
                              className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[10px] font-bold bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-500/20 hover:bg-amber-500/20 transition-colors cursor-pointer"
                              title="View integrity flags for this student"
                            >
                              <ShieldAlert className="w-3 h-3 text-amber-500" />
                              {studentFlagsMap.get(row.student.id)} flag{(studentFlagsMap.get(row.student.id) || 0) > 1 ? "s" : ""}
                            </button>
                          )}
                        </div>
