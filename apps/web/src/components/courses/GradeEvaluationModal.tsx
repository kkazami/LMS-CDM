...
import { X, Star, CheckCircle2, FileText, Link2, Send, ExternalLink } from "lucide-react";
...
export default function GradeEvaluationModal({
  ...
  function handleGradeChange(val: string) {
    if (maxPoints !== null && val !== "") {
      const num = parseFloat(val);
      if (!isNaN(num) && num > maxPoints) {
        setGrade(maxPoints.toString());
        setError(`Score clamped to maximum of ${maxPoints} points.`);
        return;
      }
    }
    setGrade(val);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative z-10 w-full max-w-2xl mx-4 bg-white dark:bg-[#141721] rounded-2xl border border-slate-200/80 dark:border-white/10 shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200/80 dark:border-white/10 bg-slate-50/50 dark:bg-[#181B26] shrink-0">
          ...
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-700 dark:hover:text-[#F0F2F8] transition-colors cursor-pointer p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-white/5"
          >
            <X className="h-5 w-5" />
          </button>
        </div>

        <div className="grid md:grid-cols-[1fr_260px] overflow-y-auto divide-y md:divide-y-0 md:divide-x divide-slate-200/80 dark:divide-white/10">
          ...
          <div className="p-6">
            ...
            <div className="mt-5 pt-4 border-t border-slate-100 dark:border-white/5 flex items-center justify-between">
              <Link
                href={`/${instituteCode}/courses/${courseId}/classwork/${itemId}/submissions`}
                className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#F97316] hover:underline"
              >
                <span>View all class submissions</span>
